 ...
 */
export function useDestructor(callback: DtorCallback, discriminator?: unknown): void;

/**
 * Whenever the `dependencies` change the destructor is called.
 */
export function useDestructor(callback: DtorCallback, dependencies: Array<unknown>, discriminator?: unknown): void;

export function useDestructor(callback: DtorCallback, dependenciesOrDiscriminator?: unknown, discriminator?: unknown) {
	if (typeIs(dependenciesOrDiscriminator, "table")) {
		...
		const storage = useHookState(dependenciesOrDiscriminator, cleanup) as Storage;
